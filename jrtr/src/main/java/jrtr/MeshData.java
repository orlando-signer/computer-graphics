...
public class MeshData {
    ...
    private void createVertexData() {
        ...
        VertexData data = new GLVertexData(vertexTable.size());
        ...
        // vertexData.addElement(getFaceNormals(), VertexData.Semantic.NORMAL,
        // 3);
    }
    ...
    public void loop() {
        ...
        Map<Edge, Vertex> newVertices = new HashMap<>();
        ...
        for (Face face : faceTable) {
            ...
            for (Edge edge : edges) {
                if (newVertices.get(edge) == null) {
                    ...
                    pos.scale(3, vertexTable.get(edge.v1).position);
                    pos.scaleAdd(3, vertexTable.get(edge.v2).position, pos);

                    List<Integer> f1 = findVertices(edge.f1);
                    List<Integer> f2 = findVertices(edge.f2);
                    f1.addAll(f2);
                    f1.stream().filter(i -> i != edge.v1 && i != edge.v2)
                            .forEach(i -> pos.add(vertexTable.get(i).position));
                    ...
                    newVertices.put(edge, v);
                    vertices.add(v);
                }
                indices.add(vertices.indexOf(newVertices.get(edge)));
            }

            List<Integer> verts = findVertices(face);

            for (Integer v : verts) {
                Vertex vertex = vertexTable.get(v);
                if (!vertices.contains(vertex)) {
                    List<Vertex> v2 = findVertices(vertex);
                    // calculate position for old vertex
                    float beta = (v2.size()) <= 3 ? 3 / 16F : 3 / (8F * v2.size());
                    vertex.position.scale(1 - v2.size() * beta);
                ...
                indices.add(vertices.indexOf(newVertices.get(vertex.edge)));
                indices.add(vertices.indexOf(vertex));
                Edge e = findAdjacentEdge(vertex, edges);
                indices.add(vertices.indexOf(newVertices.get(e)));
            }
        }

        createMesh(vertices, Ints.toArray(indices));

    }

    private Edge findAdjacentEdge(Vertex vertex, List<Edge> edges) {
        int index = vertexTable.indexOf(vertex);
        for (Edge edge : edges) {
            if (vertex.edge != edge && (index == edge.v1 || index == edge.v2))
                return edge;
        }
        throw new IllegalStateException();
    }
    ...
    protected class Vertex {
        ...
        public Vertex(Vertex vertex) {
            ...
            color = new Vector3f(vertex.color);
            normal = new Vector3f(vertex.normal);
            texCoord = new Vector2f(vertex.texCoord);
        }
    }
