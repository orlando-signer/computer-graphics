 ...
 * {@link SWVertexData} which should be instantiated via
 * {@link RenderContext#makeVertexData(int)}.
 */
public abstract class VertexData {
    ...
    public void addElement(float[] f, Semantic s, int i) {
        if (f.length == n * i) {
            ...
            System.err.printf("Array of '" + s.name()
                    + "' has not the correct dimension (must be number of vertices times i (%d * %d), was %d.\n"
                    + "No elements for " + s.name() + " have been added so far.\n", n, i, f.length);
        }
    }
